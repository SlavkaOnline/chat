...
use super::chat::{ConnectionId, ChatRef};
...
impl ChatRoomRef  {
    ...
    pub async fn add_message(&self, message: ChatMessage) {
        self.command_channel.send(ChatRoomCommand::AddMessage(message)).await;
    }

    pub async fn connect(&self, connection_id: ConnectionId, message_sender: mpsc::Sender<ChatMessage>) {
        self.command_channel.send(ChatRoomCommand::Connect {
            connection_id,
            message_sender
            }
        ).await;
    ...
    pub async fn disconnect(&self, connection_id: ConnectionId) {
        self.command_channel.send(ChatRoomCommand::Disconnect { connection_id }).await;
    }
...
impl ChatRoom {
    pub fn new(id: ChatRoomId, chat: ChatRef) -> ChatRoom {
        return ChatRoom { 
            id, 
    ...
    pub fn start(mut self) -> ChatRoomRef {
        ...
        let chat_room_ref = ChatRoomRef {command_channel: sender.clone()};
        ...
        tokio::spawn(async move {
            ...
            while let Some(msg) = receiver.recv().await {
                ...
                match msg {
                    ...
                    ChatRoomCommand::Connect { connection_id, message_sender } => {
                        self.connections.insert( connection_id, Connection {id: connection_id, message_sender: message_sender.clone()});
